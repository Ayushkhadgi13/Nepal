...
export default function IntroScreen({ onComplete }: IntroScreenProps) {
  ...
  useGSAP(() => {
    ...
    tl.fromTo(
      ...
      { opacity: 0.9, y: 0, filter: 'blur(0px)', duration: 1.3, stagger: 0.15, ease: 'power3.out' }
    );

    tl.to(charsRef.current, { opacity: 0.15, scale: 1.03, duration: 1.1, ease: 'power2.inOut' }, '-=0.5');

    tl.fromTo(contentWrapperRef.current, { opacity: 0, y: 15 }, { opacity: 1, y: 0, duration: 1.0, ease: 'power3.out' }, '-=0.8');

    tl.fromTo(lineRef.current, { scaleX: 0 }, { scaleX: 1, duration: 1.4, ease: 'power4.inOut' }, '-=1.0');

    tl.to({}, { duration: 0.75 });

    tl.to(containerRef.current, { opacity: 0, scale: 1.03, duration: 1.1, ease: 'power3.inOut' });
  }, { scope: containerRef });
    ...
    >
      ...
      <div className="relative max-w-4xl flex flex-col items-center justify-center z-10">
        ...
        <div ref={contentWrapperRef} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full mt-28 md:mt-36">
          ...
          <div ref={lineRef} className="h-[1px] w-28 bg-[#2A1D13]/15 mx-auto my-6 origin-center" />

          <p className="text-[10px] md:text-xs font-light text-[#5B4534] uppercase tracking-[0.45em] max-w-md mx-auto leading-relaxed italic">
            "An Archival Expedition Upward"
          </p>
        </div>
      </div>
    </div>
